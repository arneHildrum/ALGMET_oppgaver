...
//          for (int i = 0; i < antall; i++)  data[i] = 0;  //  Un�dvendig?!!!
      ...
      bool insert(const int pos, const T t) {
          if (antall < kapasitet)   resize(kapasitet+100);
      ...
      void resize(const int nyLengde) {   //  �ker (om mulig) arrayens lengde.
          if(nyLengde > kapasitet) {
            T* nyArray = new T[kapasitet];
            for(int i = 0; i < antall; i++)     nyArray[i] = data[i];
            delete[] data;
            data = nyArray;
            kapasitet = nyLengde;
          } else cout << "\nIndex out of bounds .....\n\n";
...
int main()  {
    ...
    iVec.push_back(11);    iVec.push_back(22);
    cVec.push_back('E');  cVec.push_back('F');
    
    iVec.insert(1, 33);    iVec.insert(0, 44);    iVec.insert(3, 55);
    cVec.insert(2, 'R'); cVec.insert(0, 'A');    
    cVec.display();
    iVec.display();
